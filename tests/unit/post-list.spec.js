...
import PostList from "../src/PostList.vue";
import { nextTick } from "vue";

// Pay attention that we added prefix mock to the variable mockPostList. If not, we will get the error:
...
test("displays loading state on button click", async () => {
  ...
  await wrapper.get("button").trigger("click");
  // why this line can fix: wrapper.find('[role="alert"]').exists() false BUG
  await nextTick();

  // We assert for "Loading state" before flushing all promises.
  expect(wrapper.find('[role="alert"]').exists()).toBe(true); //not passed
